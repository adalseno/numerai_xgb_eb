...
if model_selection_loop:
    ...
    targets = ["target", "target_nomi_60", "target_ben_20"]
...
"""
On a 32GB machine stop here if model selection loop is set to true. 
This will use about 90% of your memory if you use int8 training data.
...
download_data(napi, "numerai_tournament_data_int8.parquet", f"numerai_tournament_data_int8_{current_round}.parquet")
print("downloading validation_data")
download_data(napi, "numerai_validation_data_int8.parquet", f"numerai_validation_data_int8.parquet")
...
validation_data["prediction"].to_csv(f"validation_predictions_{current_round}.csv", index=True)
tournament_data["prediction"].to_csv(f"tournament_predictions_{current_round}.csv", index=True)